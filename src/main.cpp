...
int getSensorFusionVehicleLane(float d){
  if (d > 0 && d < 4) {
    return 0;
  }
  else if (d > 4 && d < 8){
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            for (int i = 0; i < sensor_fusion.size(); i++) {
              ...
              int vehicle_lane = getSensorFusionVehicleLane(d);
              if (vehicle_lane != -1){
                // find vehicle speed
                ...
                // Estimate vehicle s position after executing previous trajectory.
                check_vehicle_s += ((double)prev_size*0.02*check_speed);

                if (vehicle_lane == lane){
                  // Vehicle in our lane.
                  vehicle_ahead |= check_vehicle_s > car_s && check_vehicle_s - car_s < 30;
                }
                else if ((vehicle_lane - lane) == -1){
                  // Vehicle left
                  vehicle_left |= car_s - 40 < check_vehicle_s && car_s + 40 > check_vehicle_s;
                }
                else if ((vehicle_lane - lane) == 1){
                  // Car right
                  vehicle_right |= car_s - 40 < check_vehicle_s && car_s + 40 > check_vehicle_s;
                }
              }
            }
